...
public class RemoverMedico extends javax.swing.JFrame {
    ...
    private void TabelaListaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_TabelaListaMouseClicked
        ...
        int input = JOptionPane.showConfirmDialog(null,
            "Deseja eliminar este médico?", "Selecione uma opção",JOptionPane.YES_NO_OPTION);

        if(input == 0){
            try {
                medController.destroy(med.getId());
                ucontroller.destroy(med.getIdUser().getId());
                JOptionPane.showMessageDialog(null, "Médico eliminado.");
                GerirMedico gm = new GerirMedico();
                gm.setVisible(true);
                dispose();
            } catch (NonexistentEntityException e) {
                JOptionPane.showMessageDialog(null, "Problema a eliminar médico.");
            }
        }
